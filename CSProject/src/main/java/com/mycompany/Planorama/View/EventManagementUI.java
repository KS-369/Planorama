...
import com.mycompany.Planorama.View.BackToHomeBtn;
import com.mycompany.Planorama.View.EditEventListener;
import com.mycompany.Planorama.View.DeleteEventListener;
import com.mycompany.Planorama.Controller.Controller;
import com.mycompany.Planorama.Model.Event;
import static com.mycompany.Planorama.View.HomePageUI.currentUser;
import javax.swing.*;
import java.awt.*;
...
/** TESTING
 * 1. Passing a random word into the title section of the page.
 * PASSES - saves to the db and displays the title given, and empty fields for the date and description.
 * 2. Updating the title by inputing a random date (may 22) in the date field.
 * PASSES - saves to the db, updates and displays the event, and changes the "update" button back to "save" button.
 * 3. Updating the title and the date given, by inputing a description.
 * PASSES - saves to the db, updates the event by adding the description to the eventListPanel, and changes the "update" button back to "save" button.
 * 4. Inputing the same title that is already saved to re-save it again as a new event. 
 * PASSES - throws an error in the console stating that the event already exists and can't be created.
 * 5. created two usernames (user and otheruser) and created two different events under each user (sri: my event; otheruser: first event).
 * PASSES - Logged out and re-logged in with each username and it displays and saves the events that each user created (my event for sri; first event for otheruser).
 *        - saves events for each user under their username.
 * 6. Passing empty fields into the title, date, and description field inputs.
 * PASSES - displays an error message on the screen ensuring those fields are not empty!
*/
